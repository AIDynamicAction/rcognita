...
parser.add_argument(
    "--model_order", type=int, default=5, help="Order of state-space estimation model."
)
parser.add_argument(
    "--prob_noise_pow",
    type=float,
...
my_ctrl_nominal = controllers.CtrlNominal3WRobot(
    m, I, ctrl_gain=5, ctrl_bnds=ctrl_bnds, t0=t0, sampling_time=dt
)

state_predictor = controllers.EulerStatePredictor(
    pred_step_size, my_sys._state_dyn, my_sys.out, dim_output, Nactor
)

# Predictive optimal controller
my_ctrl_opt_pred = controllers.CtrlOptPred(
    dim_input,
    dim_output,
    ctrl_mode,
    ctrl_bnds=ctrl_bnds,
...
for k in range(0, Nruns):
    ...
    if is_log_data:
        ...
        with open(datafiles[k], "w", newline="") as outfile:
            ...
            writer.writerow(
                ["model_est_period_multiplier", str(model_est_period_multiplier)]
            )
            ...
            writer.writerow(
                ["pred_step_size_multiplier", str(pred_step_size_multiplier)]
            )
...
if is_visualization:
    ...
    while True:
        ...
        action = controllers.ctrl_selector(
            t, observation, action_manual, my_ctrl_nominal, my_ctrl_benchm, ctrl_mode
        )
