...
class Obstacles_parser:
    ...
    def get_buffer_area(self, line, buf = None):
        if buf is None:
            buf = self.W / 2
        p1, p2 = line
        if (p1[0] > p2[0]):
    ...
    def get_functions(self):
        ...
        def get_circle_constraints(circle):
            center = circle.center
            r = circle.r
            return lambda x : r**2 - (center[0] - x[0])**2 - (center[1] - x[1])**2
        ...
        if len(self.lines) > 0:
            ...
            for ineq in inequations:
                func_constr = get_constraint(ineq)
                constraints.append(func_constr)
        
        if len(self.circles) > 0:
            for circle in self.circles:
                func_constr = get_circle_constraints(circle)
                constraints.append(func_constr)

        constraints = list(({'type': 'ineq', 'fun': constr}) for constr in constraints)
        return constraints
...
class ROS_preset:
    ...
    def laser_scan_callback(self, dt):
        ...
        self.constraints = self.obstacles_parspython3 main_3wrobot_ros_obst3.py --ctrl_mode MPC --dt 0.07 --Nactor 5 --pred_step_size_multiplier 6 --buffer_size 200 --Ncritic 80 --critic_period_multiplier 5 --model_est_stage 2

er(np.array(dt.ranges))
